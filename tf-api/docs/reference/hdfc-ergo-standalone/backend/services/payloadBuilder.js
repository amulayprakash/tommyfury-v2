...
function policyDetails_Rollover(req, opts = {}) {
  ...
  // Four wheeler = product 2311; POLICY_TYPE ("TP Only"/"OD Plus TP") ने policy
  // ठरte. prevPolicyType Blaze schema la VALID non-empty lagto (rikama/Liability
  // schema todto -> 'prevPolicyStartDate expected prevPolicyType' error). Mhaнun
  // nehmi "Comprehensive Package" (Roll Over sara challela).
  // frontend "COMPREHENSIVE" (all caps) pathvto — Blaze la "Comprehensive Package"
  // havа. Normalize: कोणतेही comprehensive variant -> "Comprehensive Package".
  const normalizePrevType = (t) => {
    const s = String(t || '').trim().toLowerCase();
    if (!s || s.includes('comprehensive')) return 'Comprehensive Package';
    if (s.includes('liability') || s.includes('tp')) return 'Comprehensive Package'; // TP Only पण prev comprehensive ठेवतो (valid)
    return t; // इतर काही असल्यास तसंच
  };
