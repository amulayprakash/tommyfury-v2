...
**Data layer:** Prisma + MySQL (`prisma/schema.prisma`). Masters (`RtoMaster`, `MmvMaster`, `InsurerMaster`, `MotorAddon`) are seeded; **FG is the master data source**, so `MmvMaster.modelId` holds FG's PASIA_CODE and rows carry FG enrichment (GVW/seating/vehicle class). `Provider*Code` tables map canonical master IDs → vendor codes. `db-code-resolver.ts` does this translation in production; `passthroughCodeResolver` is used in dev/fixtures. **Imports are idempotent and partition-scoped:** each canonical row carries a `source` (`fg`/`icici`) and is **upserted** (never wiped), so re-running one provider's import can't destroy another's codes — import order no longer matters. `ProviderRtoCode` is **line-aware** (`line` = tw/fw/cv, ICICI has different RTO codes per vehicle line). ICICI codes are best-effort cross-walk matches; `scripts/validate-icici-codes.ts` verifies them against live UAT (`verifiedAt`/`verifyError`) and deletes only definitively-rejected ones. Schema changes go through Prisma migrations (one squashed `0_init` baseline; use `npm run db:migrate`). **All monetary amounts (premiums, IDV, payment) are whole rupees (INR), stored as integers** — the whole stack (provider normalizers → DB → FG payment gateway → frontend `formatInr`) uses rupees end-to-end; there is no paise conversion anywhere.

**Config & conventions:** `src/config/env.ts` validates all env via zod and fails fast at boot, and it holds **server wiring + secrets ONLY**. **Vendor credentials are env-only — never in the DB or committed code**; see `tf-api/.env.example` for the full set. Everything else a vendor needs — gateway hosts, product/agent/partner codes, token TTLs, payment options, and the `*_ENABLED` toggle that registers the provider — is provider *data* and lives in code beside the adapter (`FG_GATEWAY`/`FG_CHANNEL`/`FG_PAYMENT` in `providers/fg/config.ts`, `ICICI_GATEWAY`, `HDFC_GATEWAY`, `ITGI_GATEWAY`), with our own origins for vendor callbacks/returns in `src/config/app-urls.ts`. **Those values are NOT env-overridable** — config is authoritative, so changing a host means editing the provider's `config.ts`. The current values are UAT; production hosts are still open confirmations with each vendor.
